...
Item {
  ...
  // Custom bars are instantiated through a dynamic Loader, whose host values
  // arrive in onLoaded. Defaults let construction finish before that injection.
  ...
  component BarPanel: PanelWindow {
    ...
    // The custom bar uses separate island surfaces below. Keep the layer
    // window itself clear so the wallpaper shows through between them.
    color: "transparent"
    ...
    Component {
      ...
      Item {
        ...
        // Three separate islands: workspace controls, focused-window title,
        // and system status. Their tops extend past the layer surface so only
        // the lower corners read as rounded, matching a notched top edge.
        BorderSurface {
          z: -1
          anchors.left: leftModules.left
          anchors.leftMargin: -Style.space(4)
          anchors.right: leftModules.right
          ...
          borderSpec: Border.flat(Qt.rgba(root.barForeground.r, root.barForeground.g, root.barForeground.b, 0.12), 1)
          radius: Style.space(10)
        }

        BorderSurface {
          z: -1
          readonly property real contentLeft: centerModules.visibleContentLeft
          readonly property real contentRight: centerModules.visibleContentRight
          readonly property real contentWidth: Math.max(0, contentRight - contentLeft)

          // Center the island on what the widgets actually paint. The clock is
          // screen-centered while weather sits to its right, so centering the
          // island on the screen leaves visibly more empty space on the left.
          width: Math.max(Style.space(154), contentWidth + Style.space(12))
          // Optical correction: the clock label's leading inset is slightly
          // wider than the weather glyph's trailing inset, so nudge the
          // island right to equalize the painted-content margins.
          x: (contentLeft + contentRight - width) / 2 + Style.space(2)
          anchors.top: parent.top
          anchors.topMargin: -Style.space(8)
          anchors.bottom: parent.bottom
          anchors.bottomMargin: Style.space(2)
        ...
        BorderSurface {
          z: -1
          x: rightModules.x + rightModules.leadingHiddenExtent - Style.space(4)
          width: Math.max(0, rightModules.width - rightModules.leadingHiddenExtent + Style.space(8))
          anchors.top: parent.top
          anchors.topMargin: -Style.space(8)
          anchors.bottom: parent.bottom
          anchors.bottomMargin: Style.space(2)
  ...
  component CenterModules: Item {
    ...
    readonly property real visibleContentLeft: {
      var content = centerLoader.item
      if (root.vertical || !content || !("visibleContentLeft" in content)) return width / 2
      return Number(content.visibleContentLeft)
    }
    readonly property real visibleContentRight: {
      var content = centerLoader.item
      if (root.vertical || !content || !("visibleContentRight" in content)) return width / 2
      return Number(content.visibleContentRight)
    }
    ...
    Component {
      ...
      Item {
        anchors.fill: parent
        readonly property real visibleContentLeft: centerRoot.hasAnchor
          ? centerBeforeModules.x
          : centeredModules.x
        readonly property real visibleContentRight: centerRoot.hasAnchor
          ? centerAfterModules.x + centerAfterModules.width
          : centeredModules.x + centeredModules.width
  ...
  component ModuleList: Loader {
    ...
    // The tray reserves its whole drawer width even while collapsed so its
    // icons can slide in without moving the rest of the bar. Island chrome
    // should follow only the visible part of that reservation.
    readonly property real leadingHiddenExtent: {
      if (root.vertical || region !== "right") return 0

      var slots = root.moduleSlots
      var listWindow = root.targetWindow(moduleListRoot)
      for (var i = 0; i < slots.length; i++) {
        var slot = slots[i]
        if (!slot || slot.region !== region || !slot.activeItem) continue
        if (!root.sameWindow(root.slotWindow(slot), listWindow)) continue

        var widget = slot.activeItem
        if (!("drawerExtent" in widget) || !("revealExtent" in widget)) continue
        return Math.max(0, Number(widget.drawerExtent) - Number(widget.revealExtent))
      }
      return 0
    }
